...
- 클라이언트가 일정 주기마다 서버에 요청
- 서버는 매 요청마다 Redis 데이터를 조회
- 데이터 변경 여부와 관계없이 반복 호출 발생
...
- Redis 데이터 변경 시에만 서버에서 이벤트 발생
- Server-Sent Events(SSE)를 통해 클라이언트로 실시간 전송
- 클라이언트는 연결을 유지하며 이벤트 수신

#### 개선 이유

- Redis 변경이 없을 경우 발생하던 불필요한 Polling 요청 제거
- Redis 및 서버 부하 감소로 성능 및 확장성 향상
- 데이터 변경 즉시 클라이언트에 전달되어 실시간성 강화
- 이벤트 기반 구조로 전환하여 클라이언트 로직 단순화
- 서버 중심 이벤트 제어로 유지보수성 및 가독성 개선
